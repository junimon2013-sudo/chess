...
function redrawAnnotations() {
  ...
  layer.innerHTML = `<defs><marker id="${markerId}" markerWidth="8" markerHeight="8" refX="4" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="${ANNOT_COLOR}" /></marker></defs>`;
...
// Sidebar shows the children of the last node in the path (or root's children if
// path only has the root). Selecting a leaf/named node with no further named
// descendants just shows its own children as the list (which may be empty).
let openingRootKey = 'e4';
let openingPath = []; // [rootNode, ...selectedChildren]

document.getElementById('btn-openings').addEventListener('click', () => {
  showScreen('screen-openings');
  openingRootKey = 'e4';
  openingPath = [OPENING_TREE.e4];
  document.querySelectorAll('.opening-root-tab').forEach(t => t.classList.toggle('selected', t.dataset.root === 'e4'));
  renderOpeningScreen();
});

document.querySelectorAll('.opening-root-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    openingRootKey = tab.dataset.root;
    openingPath = [OPENING_TREE[openingRootKey]];
    document.querySelectorAll('.opening-root-tab').forEach(t => t.classList.toggle('selected', t === tab));
    renderOpeningScreen();
  });
});
...
document.getElementById('opening-back-btn').addEventListener('click', () => {
  ...
  // Pop back to the previous *named* node (or the root if none), so back
  // always lands on a meaningful branch point rather than a mid-chain move.
  openingPath.pop();
  while (openingPath.length > 1 && !openingPath[openingPath.length - 1].name) {
    openingPath.pop();
  }
  renderOpeningScreen();
});

// Collects the chain of SAN moves from root to the current path (excluding the
// synthetic root wrapper's own san, which IS the first real move).
function openingPathSANs() {
  return openingPath.map(n => n.san);
}

// Returns the "named children list" to show in the sidebar. Named nodes are
// shown directly. An unnamed node (an intermediate forced/common move) is not
// shown itself — instead we recurse into ITS children and collect those, so the
// sidebar always lists meaningfully named branches, never raw half-moves.
// Each returned entry carries `viaPath`: the full chain of nodes from the current
// position down to that named node, so selecting it appends all of them at once.
function collectNamedChildren(node, pathSoFar) {
  const results = [];
  for (const child of (node.children || [])) {
    const childPath = pathSoFar.concat([child]);
    if (child.name) {
      results.push({ node: child, viaPath: childPath });
    } else {
      results.push(...collectNamedChildren(child, childPath));
    }
  }
  return results;
}

function openingSidebarChildren() {
  const current = openingPath[openingPath.length - 1];
  return collectNamedChildren(current, []);
}
...
function buildGameForPath(path) {
  ...
  for (const node of path) {
    ...
    const mv = g.allLegalMoves().find(m => {
      ...
      return rec && rec.san.replace('+','').replace('#','') === sans.replace('+','').replace('#','');
    });
    if (!mv) break;
...
function renderOpeningScreen() {
  ...
  // right title: show the move sequence in algebraic form
  const moveLine = [];
  for (let i = 0; i < openingPath.length; i++) {
    const n = openingPath[i];
    if (i % 2 === 0) moveLine.push((Math.floor(i / 2) + 1) + '.' + n.san);
    else moveLine.push(n.san);
  }
  document.getElementById('opening-right-title').textContent = moveLine.join(' ');

  renderOpeningBoard(g);
  renderOpeningArrows(current.threats || []);

  // sidebar list: children of current node
  const listEl = document.getElementById('opening-node-list');
  listEl.innerHTML = '';
  const children = openingSidebarChildren();
  if (children.length === 0) {
    ...
    children.forEach(entry => {
      ...
      item.textContent = entry.node.name;
      item.addEventListener('click', () => {
        openingPath.push(...entry.viaPath);
        renderOpeningScreen();
      });
  ...
  svg.innerHTML = `
    <defs>
      <marker id="arrowhead" markerWidth="8" markerHeight="8" refX="4" refY="4" orient="auto">
        <path d="M0,0 L8,4 L0,8 Z" fill="rgba(201,162,75,0.85)" />
      </marker>
    </defs>
  `;
